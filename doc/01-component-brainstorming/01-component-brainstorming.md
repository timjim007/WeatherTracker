...
My career goal is to be an embedded engineer, working with both software and hardware. Some of my personal hobbies are I love soccer, I love playing video games in my free time, recently started playing valorant.
...
- Component Design #1: `Weather`
  - **Description**: The purpose of this component is to model a weather information system. The intent of this design is to keep a simple kernel that fetches weather data from an API and displays it to the user. Then, we provide more operations in the secondary interface such as current weather, forecast for the next few days, location-based weather, and alerts for severe weather conditions.
  - **Kernel Methods**:
  `double getTempInF()` : gets temperature of `this` in Fahrenheit
  `double getTempInC()` : gets temperature of `this` in Celsius
  `String getForecast()` : gets weather forcast of `this`
  `String getLocation()` : gets location of `this`
  `double setTempInF(double x)`: sets temperature of `this` in Fahrenheit
  `double setTempInC(double y)`: sets temperature of `this` in Celsius
  `String setForecast(String z)`: sets weather forcast of `this`
  `String setLocation(String a)` : sets location of `this`
- **Secondary Methods**:
- `void currentWeather()`: prints weather details
- `void Forecast(String location)`: prints weather forcast based on location
- `void weatherAlerts()`: prints any weather alerts.
