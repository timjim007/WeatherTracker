...
My representation by convention : $this.rep is a Map and $this.rep is {(location,{(key,value)})} where key is (temperature,humidity,windspeed, precipitation ,condition) and value is the number or string associated with the key

By Correspondence: this = {(location,{(key,value)})}
